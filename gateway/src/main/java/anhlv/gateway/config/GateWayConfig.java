...
public class GateWayConfig {
    ...
    // Cấu hình limit cho mỗi IP
    private final Bandwidth ipLimit = Bandwidth.classic(5, Refill.intervally(5, Duration.ofSeconds(1)));
    ...
    public GlobalFilter globalAuthenticationFilter() {
        return (exchange, chain) -> {
            ...
            // Bỏ qua các API không cần xác thực (đăng nhập, đăng ký, chuyển hướng)
            if (path.equals("/api/auth/login") || path.equals("/api/auth/register") || path.equals("/api/auth/redirect")) {
            ...
            try {
                ...
                logger.error("Invalid token signature: token={}, method={}, path={}", authToken, method, path, e);
                // Token không hợp lệ (signature không khớp)
                throw new CustomException(401, "Token không hợp lệ");
            } catch (io.jsonwebtoken.ExpiredJwtException e) {
                // Token đã hết hạn
                logger.error("Expired token: token={}, method={}, path={}", authToken, method, path, e);
                throw new CustomException(401, "Token đã hết hạn");
            } catch (io.jsonwebtoken.MalformedJwtException e) {
                // Token bị hỏng
                logger.error("Malformed token: token={}, method={}, path={}", authToken, method, path, e);
                throw new CustomException(401, "Token bị hỏng");
            } catch (Exception e) {
                // Lỗi tổng quát
                logger.error("Unexpected token decode error: token={}, method={}, path={}", authToken, method, path, e);
    ...
    public RouteLocator customRoutes(RouteLocatorBuilder builder) {
        return builder.routes()
                .route("auth-service", r -> r.path("/api/auth/**")
                        .filters(f -> f.cacheRequestBody(String.class)) // Cache request body
                        .uri(authServiceUri)) // Sử dụng tên đăng ký với Eureka
                .route("face-service", r -> r.path("/api/face/**")
                        .filters(f -> f.cacheRequestBody(String.class)) // Cache request body
                        .uri(faceServiceUri))
                // Business service đã được merge vào face-service - point business endpoints đến face-service
